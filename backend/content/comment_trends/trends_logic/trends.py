...
def compute_trends():
    ...
    for tag in tags:
        ...
        trends_logger.info(
            "разделе %s %s трендовых тем и %s трендовых видео",
            tag,
            len(theme_result),
            len(video_result),
        )
...
def get_theme_image(theme_id, used, field):
    response = CollectionRequest.get_response(
        collection_id=theme_id, offset=0, limit=20
    )
...
def get_potential_trends(tag, feed_params):
    ...
    for carousel_id in carousels:
        new_documents, feed_params["cache_hash"] = get_documents_from_carousel(
            carousel_id, feed_params
        )
...
def get_documents_from_carousel(carousel_id, feed_params):
    response = CarouselRequest.get_response(
        carousel_id, offset=0, limit=feed_params["num_docs"]
    )
...
def group_comments_by_themes(documents, doc_to_comments) -> Dict[Tuple[str, str], List]:
    ...
    for doc_id in documents:
        ...
        for theme in themes:
            themes_to_comments[(theme["id"], theme["title"])].extend(
                doc_to_comments[doc_id]
            )
