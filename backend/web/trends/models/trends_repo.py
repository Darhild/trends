...
class Repository:
    ...
    def _date_validation(date):
        ...
        if now - delta <= date <= now + delta:
            ...
            logging.getLogger(__name__).warning(
                "Google has returned invalid date. "
    ...
    def video_record_row_to_dict(video_rec):
        ...
        for trend in video_rec[0]:
            ...
            result.append({"day": int(comments_count), "data": d, "title": d["title"]})
        return result
    ...
    def trend_record_row_to_dict(trend_rec, source):
        ...
        for trend in trend_rec[0]:
            ...
            day = d.pop("day")
            d["source"] = source
            result.append({"day": day, "data": d, "title": d["title"]})
        return result
        ...
        with self.db.begin() as conn:
            with conn.begin():
                ...
                logging.getLogger(__name__).debug(
                    "Got new google trends for %s date", date
                )

                delete_stmt = trend_table.delete().where(
                    cast(trend_table.c.created_at, Date) == date.date()
                )
                result = conn.execute(delete_stmt)
                data["created_at"] = date
                conn.execute(trend_table.insert(), **data)
                logging.getLogger(__name__).debug(
                    "%s entry were updated in trend_table", result.rowcount
                )
    ...
    def insert_content(self, content_json):
        with self.db.begin() as conn:
            with conn.begin():
                ...
                logging.getLogger(__name__).debug(
                    "%s entry were updated in content_table", result.rowcount
                )
    ...
    def insert_video(self, video_json):
        with self.db.begin() as conn:
            with conn.begin():
                ...
                logging.getLogger(__name__).debug(
                    "%s entry were updated in video_table", result.rowcount
                )
        ...
        with self.db.begin() as conn:
            with conn.begin():
                ...
                logging.getLogger(__name__).info(
                    "google trends num rows: {0}".format(len(result))
                )
                return result
    ...
    def read_content(self, period, tag):
        with self.db.begin() as conn:
            with conn.begin():
                ...
                logging.getLogger(__name__).info(
                    "efir trends num rows: {0}".format(len(result))
                )
    ...
    def read_videos(self, period, tag):
        with self.db.begin() as conn:
            with conn.begin():
                ...
                result = group_by_title(result)
                logging.getLogger(__name__).info(
                    "efir videos num rows: {0}".format(len(result))
                )
                return result
