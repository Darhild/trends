...
from trends.handlers.trends_algs import merge, sort, sort_and_limit
...
def trends_handler():
    ...
    try:
        logging.getLogger(__name__).info(
            "New request, tag:{0}, num_docs:{1}, period:{2}, source:{3}".format(
        ...
        if source == "efir":
            ...
            if tag is None:
                ...
                return Response(
                    response=json.dumps("Google do not have tags", ensure_ascii=False),
                    status=400,
            ...
            if tag is None:
                ...
                efir_trends = [
                    x["data"] for x in sort(repo.read_content(period, "common"))
                ]
                resp = merge(
                    efir_trends, google_trends, internal_ratio, external_ratio, num_docs
                )
            else:
                resp = sort_and_limit(repo.read_content(period, tag), num_docs)
            shuffle(resp)
...
def videos_handler():
    ...
    try:
        logging.getLogger(__name__).info(
            "New request, tag:{0}, num_docs:{1}, period:{2}".format(
                tag, num_docs, period
            )
        )
